...
namespace edm {
  ...
  InputFileCatalog::InputFileCatalog(std::vector<std::string> configuredFileNames,
    ...
    for (auto& configuredFileName : configuredFileNames_) {
      ...
      if (notLogicalFileName(configuredFileName)) {
  ...
  bool InputFileCatalog::notLogicalFileName(std::string const& configuredFileName) {
  ...
  std::vector<std::string> InputFileCatalog::physicalFileNames(std::string const& configuredFileName) const {
    ...
    bool notLFN = notLogicalFileName(configuredFileName);
    unsigned int numberOfPFNs = (notLFN || overrideFileLocator_) ? 1 : fileLocators_.size();
    for (unsigned int iCatalog : std::views::iota(0u, numberOfPFNs)) {
      pfns.push_back(physicalFileName(iCatalog, configuredFileName, notLFN));
    }
    return pfns;
  }
  ...
  std::string InputFileCatalog::firstPFNFromFirstCatalog() const {
    ...
    bool notLFN = notLogicalFileName(firstConfiguredFileName);
    return physicalFileName(iCatalog, firstConfiguredFileName, notLFN);
  }
